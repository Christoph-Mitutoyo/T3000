% Hinweis: Um das finale Dokument zu erstellen, als weitere Option "final" angeben -> 12pt,final
% Dies versteckt u.a. TODO-Elemente
\documentclass[12pt]{report}
%für finale Version
%documentclass[12pt,final]{report}
\usepackage{style}
\usepackage{dhbwTitlepage} % Eigene Titelseite verwenden
\usepackage{censor}
% uncomment the following line for intern documents
% \StopCensoring

%Code-Formatierung
\usepackage{listings}
\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.03,0.6,0.18}
\definecolor{lgray}{rgb}{0.2,0.2,0.2}
\definecolor{lorange}{rgb}{1,0.7,0.37}
\definecolor{purple}{rgb}{0.8,0.4,1}

\lstset{frame=tb,
  aboveskip=7mm,
  belowskip=7mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle=\linespread{0.9}\small\ttfamily,
  numbers=none,
  numberstyle=\tiny\color{gray},
  identifierstyle=\color{lgray},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgreen},
  stringstyle=\color{mauve},
  breaklines=true,
  breakatwhitespace=false,
  tabsize=3,
  postbreak=\mbox{\textcolor{blue}{$\hookrightarrow$}\space},
}


% "Metadaten"
\title{\Large{Evaluierung von Tools zum Auffinden von Undefined Behavior}}
\project{T3000}
\author{Christoph Böhringer}
\supervisor{Dipl.-Inform. (FH) Thomas Weller}
\studentNumber{3275565}
\class{TINF18-IN}
\company{Mitutoyo CTL Germany GmbH}
\date{21.06.2021}

% Initialisierungen für Abkürzungsverzeichnis
\makenoidxglossaries
\loadglsentries{Acronyms.tex}

\addbibresource{literature.bib}

\begin{document}

\pagenumbering{Roman}

\maketitle
\include{tex/Erklaerung}
\include{tex/Abstract/Abstract}
\tableofcontents
\listoffigures
\listoftables
\printnoidxglossary[type=\acronymtype, title={Abkürzungen}, nogroupskip]
\printnoidxglossary[type=main]
%%%%%%%%%%%%% Inhalt %%%%%%%%%%%%%

\clearpage
\pagenumbering{arabic}

\include{tex/Einleitung/Einleitung}
\include{tex/SmallTalk/Smalltalk}
\include{tex/Stand-der-Technik/stand-technik}
\include{tex/Requirements/Requirements}
\include{tex/Loesungsfindung/loesungsfindung}
\include{tex/Loesungsfindung/loesungsbewertung}
\include{tex/Umsetzung/umsetzung}
\include{tex/Praxistests/Praxistests}
\include{tex/Zusammenfassung_Ausblick/Zusammenfassung}
\include{tex/Zusammenfassung_Ausblick/Ausblick}
\include{tex/Anhang/Anhang}


\printbibliography          % Literaturverzeichnis
\listoftodos                % Todoverzeichnis

\end{document}